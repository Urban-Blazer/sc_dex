...
module srm_dex::quote_tests {
  ...
  use srm_dex::sc_v_eth_usdc::SC_V_ETH_USDC;
  use srm_dex::sc_s_usdc_usdt::SC_S_USDC_USDT;
  use srm_dex::curves::{Volatile, Stable};
  use srm_dex::sui_rewards_amm::{Self, Registry, SuiCoinsPool};
  use srm_dex::test_utils::{people, scenario, deploy_eth_usdc_pool, deploy_usdc_usdt_pool};
  ...
  fun test_quote_add_liquidity() {
    ...
    next_tx(test, alice);
    {
      ...
      let balance_x = sui_rewards_amm::balance_x<ETH, USDC, SC_V_ETH_USDC>(&pool);
      let balance_y = sui_rewards_amm::balance_y<ETH, USDC, SC_V_ETH_USDC>(&pool);
      let lp_coin_supply = sui_rewards_amm::lp_coin_supply<ETH, USDC, SC_V_ETH_USDC>(&pool);
